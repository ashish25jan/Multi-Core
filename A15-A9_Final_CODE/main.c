...
{	
	struct thread_param_struct *arg_struct = (struct thread_param_struct*) arg;
		...
		for( int px=20; px<width; ++px )
	{
		...
		for( int py=0; py<height; ++py )
		{
			 ...
			 */
			arg_struct->res_0[px*width + py] = scaled_color[0];
			arg_struct->res_1[px*width + py] = scaled_color[1];
			arg_struct->res_2[px*width + py] = scaled_color[2];
}
}	
free_scene( &scene );
...
main( int argc, char **argv )
{
		...
		for( int px=0; px<20; ++px )
	{
		...
		for( int py=0; py<height; ++py )
		{
			 ...
			 * but has a maximum of 70 chars/line, so use one line per pixel
			 */
			args.res_0[px*width + py] = scaled_color[0];
			args.res_1[px*width + py] = scaled_color[1];
			args.res_2[px*width + py] = scaled_color[2];
}
}	
	...
	for( int px=0; px<width; ++px )
	{
		for( int py=0; py<height; ++py )
		{
			fprintf(fp, "%.0f %.0f %.0f\n", args.res_0[px*width + py], args.res_1[px*width + py], args.res_2[px*width + py]);
		}
		fprintf(fp, "\n");
	}
	fclose(fp);
